...
$7zFilePath = Join-Path $outputDir "DeveLineStateSaver.7z"
$zipFilePath = Join-Path $outputDir "DeveLineStateSaver.zip"

DeleteFileIfExists $7zFilePath
DeleteFileIfExists $zipFilePath
DeleteFolderIfExists $outputDir

$buildPath = Join-Path $solutionRoot "DeveLineStateSaver\bin\Release\netstandard2.0"
